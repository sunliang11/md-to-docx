...
function getConfig() {
    ...
    return {
        cli: cfg.get("path", "md-to-docx"),
        preset: cfg.get("preset", "technical"),
...
function runConvert(filePath) {
    ...
    return new Promise((resolve) => {
        const proc = (0, child_process_1.spawn)(cli, args, { shell: true });
        let stderr = "";
...
function activate(context) {
    const disposable = vscode.commands.registerCommand("md-to-docx.export", async () => {
        ...
        const open = await vscode.window.showInformationMessage(`Exported ${path.basename(outDocx)}`, "Open DOCX");
        if (open === "Open DOCX") {
